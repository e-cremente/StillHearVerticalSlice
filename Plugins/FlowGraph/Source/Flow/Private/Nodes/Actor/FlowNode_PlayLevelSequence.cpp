...
FFlowNodeLevelSequenceEvent UFlowNode_PlayLevelSequence::OnSkipRequested;
...
void UFlowNode_PlayLevelSequence::SkipPlayback()
{
	if (SequencePlayer)
	{
		SequencePlayer->OnFinished.RemoveAll(this);
		FFrameTime EndFrame = SequencePlayer->GetEndTime().Time;
		SequencePlayer->SetPlaybackPosition(FMovieSceneSequencePlaybackParams(EndFrame, EUpdatePositionMethod::Play));
	}
...
void UFlowNode_PlayLevelSequence::Cleanup()
{
	if (SequencePlayer)
	{
		OnSkipRequested.RemoveAll(this);
